defmodule CuriousMessenger.ChatTest do
  ...
  describe "chat_message_reactions" do
    ...
    def mesage_reaction_fixture(attrs \\ %{}) do
      {:ok, mesage_reaction} =
        attrs
        |> Enum.into(@valid_attrs)
        |> Chat.create_mesage_reaction()

      mesage_reaction
    end

    test "list_chat_message_reactions/0 returns all chat_message_reactions" do
      mesage_reaction = mesage_reaction_fixture()
      assert Chat.list_chat_message_reactions() == [mesage_reaction]
    end

    test "get_mesage_reaction!/1 returns the mesage_reaction with given id" do
      mesage_reaction = mesage_reaction_fixture()
      assert Chat.get_mesage_reaction!(mesage_reaction.id) == mesage_reaction
    end

    test "create_mesage_reaction/1 with valid data creates a mesage_reaction" do
      assert {:ok, %MesageReaction{} = mesage_reaction} = Chat.create_mesage_reaction(@valid_attrs)
    end

    test "create_mesage_reaction/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Chat.create_mesage_reaction(@invalid_attrs)
    end

    test "update_mesage_reaction/2 with valid data updates the mesage_reaction" do
      mesage_reaction = mesage_reaction_fixture()
      assert {:ok, %MesageReaction{} = mesage_reaction} = Chat.update_mesage_reaction(mesage_reaction, @update_attrs)
    end

    test "update_mesage_reaction/2 with invalid data returns error changeset" do
      mesage_reaction = mesage_reaction_fixture()
      assert {:error, %Ecto.Changeset{}} = Chat.update_mesage_reaction(mesage_reaction, @invalid_attrs)
      assert mesage_reaction == Chat.get_mesage_reaction!(mesage_reaction.id)
    end

    test "delete_mesage_reaction/1 deletes the mesage_reaction" do
      mesage_reaction = mesage_reaction_fixture()
      assert {:ok, %MesageReaction{}} = Chat.delete_mesage_reaction(mesage_reaction)
      assert_raise Ecto.NoResultsError, fn -> Chat.get_mesage_reaction!(mesage_reaction.id) end
    end

    test "change_mesage_reaction/1 returns a mesage_reaction changeset" do
      mesage_reaction = mesage_reaction_fixture()
      assert %Ecto.Changeset{} = Chat.change_mesage_reaction(mesage_reaction)
    end
  end
